...
#     mse_var2.append(mse(y_pred[:,1], y_test[:,1]))
    ...
    result = model.fit(phi, y, 
                       validation_split = 0.1, epochs = 1000, batch_size = 512, verbose = 0)
...
def main():
    ...
    for sim in range(num_sim):
        ...
        df_loc = pd.read_csv("synthetic_data_simulations_nonGaussian/2d_nongaussian_1200_"+str(sim+1)+".csv", 
                             sep = ",")
        ...
        df_train.to_csv("synthetic_data_simulations_nonGaussian/training_data/2D_nonGaussian_1200_"+str(sim+1)+"-train.csv",
                                                              index = False)
        df_test.to_csv("synthetic_data_simulations_nonGaussian/testing_data/2D_nonGaussian_1200_"+str(sim+1)+"-test.csv",
                        index = False)
        ...
        model = model_function(df_train,phi,sim,model_saving_location)
...
#         print(y_test)
    ...
    ## MSE saved in file 
    df_mse.to_csv("plot_results/DeepKriging_nongaussian_mse.csv", index = False)
    print(mse_var1)
    print(mse_var2)
