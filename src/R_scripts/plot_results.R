...
setwd("/home/nagp/Desktop/Bivariate_DeepKriging/R_scripts/")
rm(list = ls())
...
df2 <- data.frame(Model=rep(c("CoKriging.Mate\'rn",
                              "Biv.DeepKriging point prediction CPU", 
                  ...
                  time=c(6624,22716,40824,83484,63.0,87.8,107.3,150.3,
                         23.1, 35.6, 41.8, 54.2, 
                         154 , 228,293,471,69,103,137,215))
head(df2)
df2$time = log(df2$time)
df2$Model = factor(df2$Model, levels = c("CoKriging.Mate\'rn",
                                         "Biv.DeepKriging interval prediction CPU",
                                         "Biv.DeepKriging interval prediction GPU",
                                         "Biv.DeepKriging point prediction CPU", 
                                         "Biv.DeepKriging point prediction GPU"
...
head(df2)
color_palette = colorRampPalette(colors = c("lightblue", "black"))(5)
p<-ggplot(df2, aes(x=N, y=time, group=Model)) +
  geom_line(aes(linetype=Model,color=Model),size=2.5)+
  geom_point(aes(shape=Model,color=Model),size=4.5)+ 
  labs(y= "Seconds (in log scale)", x = "Number of locations", size = 5)+ 
  theme(legend.text=element_text(size=rel(1.3)), axis.title=element_text(size=22,face="bold"),
        axis.text=element_text(size=17)) + scale_color_manual(values=color_palette)
p
...
fields::quilt.plot(newdata[,1],newdata[,2],newdata[,3],nx = 15, ny = 40, zlim = c(-3,7),
                   main = "observations: Variable 1",xaxt="n", col = myPalette(100),
                   xlab = "Longitude",ylab="Latitude", add.legend = F,
                   cex.main =1.3,lwd = 2, cex.axis = 1.2,)
...
fields::quilt.plot(newdata[,1],newdata[,2],newdata[,4],nx = 15, ny = 40,
                   main = "observations: Variable 2",col = myPalette(100),
                   xlab = "Longitude",ylab="Latitude", add.legend = F,
                   cex.main =1.3,lwd = 2,cex.axis = 1.2,)
map("worldHires", xlim = c(34.7, 35.4), ylim = c(27, 29), lwd = 1.5, add = T)
points(35.2921,27.9165, pch = ".",col = "black", bg = "yellow", cex = 10)
...
fields::quilt.plot(df$x,df$y, df$var1, main =
                   ...
                   nx = 144, ny = 144,zlim = c(-3,7),col = myPalette(100),
                   cex.main =1.3,lwd = 2,cex.axis = 1.2,legend.cex = 40)
map("worldHires", xlim = c(34.7, 35.4), ylim = c(27, 29), lwd = 1.5, add = T)
points(35.2921,27.9165, pch = ".",col = "black", bg = "yellow", cex = 10)
text(35.2,27.9165, "NEOM", font = 2,col = "black")

fields::quilt.plot(df$x,df$y, df$var2, 
                   main = "Biv.DeepKriging interpolation: Variable 2",yaxt = "n",col = myPalette(100),
                   nx = 144, ny = 144,cex.main =1.3,lwd = 2,cex.axis = 1.2,legend.cex = 40)
map("worldHires", xlim = c(34.7, 35.4), ylim = c(27, 29), lwd = 1.5, add = T)
points(35.2921,27.9165, pch = ".",col = "black", bg = "yellow", cex = 10)
text(35.2,27.9165, "NEOM", font = 2,col = "black")
