...
mle_ind_mat=function(p,data_train)
{
  ...
  {
    C11=sigma1*matern(dist.mat,sqrt(1/a1),nu1)
    C22=sigma2*matern(dist.mat,sqrt(1/a2),nu2)
...
par_Matern.loglikelihood_allcomponents=function(p,data_train){
  ...
  a12 = (a1 + a2)/2
  ######## Putting hard constraints on the parameters #############
  if( a1<=0 | nu1<=0 | sigma1<=0 | a2<=0 | nu2<=0 | sigma2<=0 |nug1<0|nug2<0 | (R^2) >1)
  {
    return(list(mlv=Inf))
  }
  else
  {
    ...
    C11=(sigma1^2)*matern(dist.mat,sqrt(1/a1),nu1)
    C22=(sigma2^2)*matern(dist.mat,sqrt(1/a2),nu2)
    C12=constant*matern(dist.mat,sqrt(1/a12),nu12)
